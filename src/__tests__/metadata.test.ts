...
describe("generateMetadata", () => {
  ...
  it("returns default metadata when optional fields are omitted", () => {
    ...
    expect(metadata.title).toBe("Abdelmalek Abed -  Data Scientist & ML Engineer");
    expect(metadata.description).toContain("AWS Solutions Architect");
    expect(metadata.metadataBase?.toString()).toBe("https://abdelmalekabed.com/");
    expect(metadata.alternates?.canonical).toBe("https://abdelmalekabed.com");
  });
  ...
  it("applies overrides and propagates image data when provided", () => {
    ...
    expect(metadata.title).toBe("About | Abdelmalek Abed");
